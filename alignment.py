...
def align_simple_branches_rec(left, right): 
    ...
    overlap_size_right = [len(set(largest_set_left) & set(o[1])) for o in right]
...
def align_simple_branches(left, right):
    ...
    for (l, r) in list_of_aligned_pairs:
        if l == None:
            ...
            result.append((l[0] + r[0], list(set(l[1] + r[1])), l[2]))
    return result
...
def align_ds(ds_map):
    ...
    for obsv_id, ds_list, pc_is_sensitive in ds_map:
        if not pc_is_sensitive:
            obsv_ids_to_base_offset_pairs_list.append(([obsv_id], ds_list, pc_is_sensitive))
            continue
