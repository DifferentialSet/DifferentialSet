 ...
 */
void do_setkey(const uint8_t *key, SEED_context *ctx, const unsigned keylen) {
    // static int initialized = 0;
    // static const char *selftest_failed = 0;
    uint32_t x1, x2, x3, x4;
    uint32_t t0_w;
    uint8_t *t0_b = (uint8_t*)&t0_w;
    uint32_t t1_w;
    uint8_t *t1_b = (uint8_t*)&t1_w;
    ...
    for (i = 0; i < NUMKC; i++) {
        t0_w = x1 + x3 - KC[i];
        t1_w = x2 + KC[i] - x4;
        *(keyout++) = SS0[t0_b[0]] ^ SS1[t0_b[1]] ^ SS2[t0_b[2]] ^ SS3[t0_b[3]];
        *(keyout++) = SS0[t1_b[0]] ^ SS1[t1_b[1]] ^ SS2[t1_b[2]] ^ SS3[t1_b[3]];
...
#define OP(X1, X2, X3, X4, rbase)                                              \
    t0_w = X3 ^ ctx->keyschedule[rbase];                                       \
    t1_w = X4 ^ ctx->keyschedule[rbase + 1];                                   \
    t1_w ^= t0_w;                                                              \
    t1_w = SS0[t1_b[0]] ^ SS1[t1_b[1]] ^ SS2[t1_b[2]] ^ SS3[t1_b[3]];                  \
    t0_w += t1_w;                                                              \
    t0_w = SS0[t0_b[0]] ^ SS1[t0_b[1]] ^ SS2[t0_b[2]] ^ SS3[t0_b[3]];                  \
    t1_w += t0_w;                                                              \
    t1_w = SS0[t1_b[0]] ^ SS1[t1_b[1]] ^ SS2[t1_b[2]] ^ SS3[t1_b[3]];                  \
    t0_w += t1_w;                                                              \
    X1 ^= t0_w;                                                                \
...
static uint8_t in_pub[16] = {0x83, 0xA2, 0xF8, 0xA2, 0x88, 0x64, 0x1F, 0xB9, 0xA4, 0xE9, 0xA5, 0xCC, 0x2F, 0x13, 0x1C, 0x7D};
static uint8_t out[16] = {0};

int main()
{
    static SEED_context ctx = {0};
    read(0, in_key, 16);
    read(0, in_pub, 16);
    do_setkey (in_key, &ctx, 16);
    do_encrypt (&ctx, out, in_pub);
