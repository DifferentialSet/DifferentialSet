...
def enumerate_solution(constraints, ctx, target_var, pub_vars, timeout=3, debug=False):
...
# %%
def do_alignment(config_dir, align_only=False):
    ...
    macro_defs, macro_defs_raw = get_ds_macro_def(config_dir, unpacked_dict)
...
if __name__ == "__main__":
    smt2_path = "issta2018-benchmarks-wu/examples/dependency_tree/cbmc.smt2"
    ...
    obsv_mapping = get_obsv_var_eq_mapping(obsv_constraints)
    branch_label_mapping = get_branch_label_mapping(branch_label_constraints)
    e = enumerate_routine(bounds_mapping, cbmc_mapping, obsv_mapping, branch_label_mapping, "Observation__0", ctx, debug=True)
    print(e)
