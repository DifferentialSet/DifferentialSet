...
benchmark_paths = []
# benchmark_paths += ["toy/3way/"]
# benchmark_paths += ["toy/secret_branch_array/"]
# benchmark_paths += ["toy/secret_and_public_branch/"]
# benchmark_paths += ["toy/secret_branch_secret_index/"]
# benchmark_paths += ["toy/secret_branch_public_index/"]
# benchmark_paths += ["toy/public_branch_public_index/"]
# benchmark_paths += ["toy/public_branch_secret_index/"]
...
# benchmark_paths += ["issta2018-benchmarks-wu/examples/appliedCryp/3way_ternary/"]
# benchmark_paths += ["issta2018-benchmarks-wu/examples/appliedCryp/des_ternary/"]
# benchmark_paths += ["issta2018-benchmarks-wu/examples/appliedCryp/loki91_ternary/"]
# benchmark_paths += ["issta2018-benchmarks-wu/examples/ghostrider/binsearch_ternary/"]
# benchmark_paths += ["issta2018-benchmarks-wu/examples/ghostrider/dijkstra_ternary/"]
# benchmark_paths += ["issta2018-benchmarks-wu/examples/ghostrider/findmax_ternary/"]
# benchmark_paths += ["issta2018-benchmarks-wu/examples/ghostrider/rsort_ternary/"]
...
for benchmark_path in benchmark_paths:
    ...
    if not do_quantification:
        ...
        with time_context(metrics_collector, "Compile mitigated.c"):
            # subprocess.run([compiler, "-O3", "-c", "mitigated.c", "-Wno-int-conversion", "-Wno-shift-op-parentheses", "-o", "mitigated.o", "-march=native"], capture_output=True, cwd=benchmark_path, check=True, env=my_env)
        ...
        with time_context(metrics_collector, "Run mitigated"):
            # result_mitigated = subprocess.run(["sudo", "cset", "shield", "--exec", "--", "perf", "stat", "-e", "cpu-cycles:u", "-x", ",", "-r", "100", "./mitigated"], stdin=open("issta2018-benchmarks-wu/random_input_large.txt"), capture_output=True, cwd=benchmark_path)
            result_mitigated = subprocess.run(["taskset", "-c", "3", "perf", "stat", "-e", "cpu-cycles:u", "-x", ",", "-r", "6000", "./mitigated"], stdin=open(home_path+"/src/benchmark/issta2018-benchmarks-wu/random_input_large.txt"), capture_output=True, cwd=benchmark_path)
        print("Run baseline program: {}".format(benchmark_name))
        with time_context(metrics_collector, "Run transform_only"):
            # result_baseline = subprocess.run(["sudo", "cset", "shield", "--exec", "--", "perf", "stat", "-e", "cpu-cycles:u", "-x", ",", "-r", "100", "./transform_only"], stdin=open("issta2018-benchmarks-wu/random_input_large.txt"), capture_output=True, cwd=benchmark_path)
            result_baseline = subprocess.run(["taskset", "-c", "3", "perf", "stat", "-e", "cpu-cycles:u", "-x", ",", "-r", "6000", "./transform_only"], stdin=open(home_path+"/src/benchmark/issta2018-benchmarks-wu/random_input_large.txt"), capture_output=True, cwd=benchmark_path)
