...
simplify_exprt::simplify_address_of(const address_of_exprt &expr)
{
  ...
  {
    // simplify &*p to p
    auto p = to_dereference_expr(new_object.expr).pointer();
    // preverse the type after simplification
    exprt cast_to_void_pointer(ID_typecast, expr.type(), {p});
    return cast_to_void_pointer;
