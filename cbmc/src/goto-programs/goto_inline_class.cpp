  ...
  const exprt::operandst &arguments)
{
  ...
  end.source_location.set_comment("end function " + id2string(identifier));
  ...
  target->source_location.set_comment("begin function " + id2string(identifier));
  target++;
  ...
  const bool force_full)
{
  ...
  Forall_goto_program_instructions(i_it, goto_program)
  {
    ...
    auto func_name = id2string(to_symbol_expr(i_it->get_function_call().function()).get_identifier());
