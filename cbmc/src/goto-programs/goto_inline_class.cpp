  ...
  const exprt::operandst &arguments)
{
  ...
  end.source_location_nonconst().set_comment("end function " + id2string(identifier));
  ...
  // kill call
  target->source_location_nonconst().set_comment("begin function " + id2string(identifier));
  ...
  const bool force_full)
{
  ...
  Forall_goto_program_instructions(i_it, goto_program)
  {
    ...
    auto func_name = id2string(to_symbol_expr(i_it->call_function()).get_identifier());
...
{
  bool b = false;
  auto func_name = id2string(id);
  // do not inline io functions
  if (func_name == "read" || func_name == "write" || func_name == "malloc" || func_name == "free"  || func_name.find("memcpy") != std::string::npos || func_name.find("memset") != std::string::npos || func_name == "strlen")
    b = true;
  return b || id == CPROVER_PREFIX "cleanup" || id == CPROVER_PREFIX "set_must" ||
