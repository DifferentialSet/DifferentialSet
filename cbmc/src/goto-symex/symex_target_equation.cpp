  ...
  decision_proceduret &decision_procedure)
{
  ...
  decision_procedure.print_annotation(";convert_assumptions\n");
  convert_assumptions(decision_procedure);
  // decision_procedure.print_annotation("convert_guards\n");
  // convert_guards(decision_procedure);
  decision_procedure.print_annotation(";convert_assignments\n");
  convert_assignments(decision_procedure);
  // decision_procedure.print_annotation("convert_goto_instructions\n");
  // // convert_decls(decision_procedure);
  // convert_goto_instructions(decision_procedure);
  // decision_procedure.print_annotation("convert_function_calls\n");
  // convert_function_calls(decision_procedure);
  // decision_procedure.print_annotation("convert_io\n");
  // convert_io(decision_procedure);
  // decision_procedure.print_annotation("convert_constraints\n");
  ...
  decision_proceduret &decision_procedure)
{
  ...
  for(auto &step : SSA_steps)
  {
    if(step.is_assignment() && !step.ignore && !step.converted)
    {
      ...
      decision_procedure.print_annotation("; " + step.source.pc->source_location().as_string());

      decision_procedure.set_to_true(step.cond_expr);
      // comment out so we can use the same equation object for multiple DIMACS files
      // step.converted = true;
  ...
  for(auto &step : SSA_steps)
  {
    if(step.is_assume())
    {
      ...
      else
      {
        ...
        // we can assume path condition is true for bounds
        auto expr_strip_condition = step.cond_expr;
        if (can_cast_expr<implies_exprt>(step.cond_expr))
        {
          expr_strip_condition =
            to_implies_expr(step.cond_expr).rhs();
        }
        decision_procedure.set_to_true(expr_strip_condition);
        // comment out so we can use the same equation object for multiple DIMACS files
        // step.converted = true;
