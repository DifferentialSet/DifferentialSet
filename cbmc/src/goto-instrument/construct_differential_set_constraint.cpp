  ...
  while (!toVisit.empty()) {
    ...
    for (auto &succ : succ_bb) {
      if (succ_bb.size() == 1) {
        ...
        auto const call = first_inst->get_function_call();
        // set up call to branch_id with ccondition_value
        program.const_cast_target(first_inst)
          ->set_function_call(code_function_callt(
            call.function(),
      ...
      if (obsv_dist[succ] == -1) {
        ...
        obsv_dist[succ] = obsv_dist[curr] + curr_count;
      } else {
        assert(obsv_dist[curr] + (int)curr_count <= obsv_dist[succ]);
      }
    }
  }
