  ...
  std::string function_name)
{
  for(auto &gf_entry : goto_model.goto_functions.function_map)
  {
    ...
    for(auto &ins : gf_entry.second.body.instructions)
    {
      ...
      auto func_name = id2string(to_symbol_expr(ins.get_function_call().function()).get_identifier());
      // do not inline io functions
      if (func_name == "read" || func_name == "write" || func_name == "memcpy" || func_name == "memset")
        continue;
  ...
  symbol_tablet &sym_table)
{
  ...
  case GOTO:
  {
    visit_to_get_mem_ops(const_cast<exprt&>(ins_it->get_condition()), log, mem_ops, sym_table);
    break;
  }
  ...
  case FUNCTION_CALL:
    ...
    // }
    {
      auto func_name = id2string(to_symbol_expr(ins_it->get_function_call().function()).get_identifier());
      assert(func_name == "write" || func_name == "read" || func_name == "branch_id" || func_name == "memcpy" || func_name == "memset");
      break;
    }
      ...
      { 
            ...
            goto_programt::instructiont::targett &ins_it)
        {
          ...
          if (mem_addr_as_void_pointer.id() != ID_symbol && macro == ID_pointer_offset) {
            ...
            exprt upper_bound_pred = binary_relation_exprt(p_offset, ID_lt, unary_exprt(ID_object_size, mem_addr_as_void_pointer, unsigned_int_type()));
