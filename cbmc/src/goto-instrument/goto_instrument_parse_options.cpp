...
/*
      // Unwind loops with upper bound computed in a separate symex
      if(cmdline.isset("get-loop-bounds")) {
        std::ofstream f("/tmp/final_loop_bounds.txt"); // Empty up this file
        f.close();
      }
*/
      ...
      if(cmdline.isset("unwind-bound")) {
        ...
        unwindset.parse_unwindset_file(config_dir+"/loop_bounds.txt");
      }
      goto_unwindt goto_unwind;
      ...
      for(auto &gf_entry : goto_model.goto_functions.function_map)
      {
            ...
            ins_it != gf_entry.second.body.instructions.end(); ins_it++)
        {
          if (ins_it->is_assume() ||
            ...
            (ins_it->is_decl() && has_prefix(id2string(ins_it->get_decl().symbol().get_identifier()), "cbmc_pointer")) ||
            (ins_it->is_dead() && has_prefix(id2string(ins_it->get_dead().symbol().get_identifier()), "cbmc_pointer"))
            ) {
            ins_it->turn_into_skip();
          }
        }
