...
void goto_unwindt::unwind(
  ...
  const unwind_strategyt unwind_strategy,
  const std::set<std::string> &constant_bound_loopids)
{
      ...
      i_it!=goto_program.instructions.end();)
  {
    ...
    if (constant_bound_loopids.find(id2string(loop_id)) != constant_bound_loopids.end()) {
      assert(loop_head->is_goto());
      goto_program.const_cast_target(loop_head)->turn_into_skip();
    }
  ...
  const std::set<std::string> &constant_bound_loopids)
{
  for(auto &gf_entry : goto_functions.function_map)
  {
    ...
    unwind(gf_entry.first, goto_program, unwindset, unwind_strategy, constant_bound_loopids);
  }
}
