...
void smt2_convt::convert_expr(const exprt &expr)
{
          ...
          expr.id()==ID_bitnor)
  {
    ...
    for (size_t i = 0; i < expr.operands().size()-1; i++) {
      out << " (";
      out << op << " ";
      flatten2bv(expr.operands()[i]);
    }

    out << " ";
    flatten2bv(expr.operands().back());
...
void smt2_convt::find_symbols(const exprt &expr)
{
  ...
  {
    // Assume our benchmark doesn't contain string constants.
    // Comment our this code because string constants all come from branch id
    // if(defined_expressions.find(expr)==defined_expressions.end())
    // {
